...
class RS_CMP200:
    ...
    def per_data_querry(self):
        ...
        while ( is_running!= 'RDY\n' ):
            is_running = self.CMP200.query('FETC:BLU:SIGN1:RXQ:PER:STAT? ')
    ...
    def non_signaling_get_data(self):
        #process data
        if ( self.testmode == 1 ):
            recive_data = self.CMP200.query('FETC:BLU:MEAS1:MEV:MOD:LEN:MAX? ')

        elif( self.testmode == 2 ):
            recive_data = self.CMP200.query('FETC:BLU:MEAS1:MEV:MOD:LEN:LE2M:MAX? ')

        elif( self.testmode == 4 ):
            recive_data = self.CMP200.query('FETC:BLU:MEAS1:MEV:MOD:LEN:LELR:MAX? ')
        return recive_data
    ...
    def offset_carrier_get_data(self):
        ...
        if ( self.testmode == 1 ):
            ...
            recive_data = self.CMP200.query('FETC:BLU:MEAS1:MEV:MOD:LEN:LRAN:MAX? ')
        print(recive_data)
    ...
    def modulation_get_data(self):
        ...
        if ( self.testmode == 1 ):
            ...
            self.cmd_send(self=self, cmd='INIT:BLU:MEAS1:MEV ')
            recive_data1 = self.CMP200.query('FETC:BLU:MEAS1:MEV:MOD:LEN:AVER? ')
            time.sleep(0.1)
            self.cmd_send(self=self, cmd='CONF:BLU:SIGN1:CONN:PACK:PATT:LEN P11 ')
            self.cmd_send(self=self, cmd='INIT:BLU:MEAS1:MEV ')
            recive_data2 = self.CMP200.query('FETC:BLU:MEAS1:MEV:MOD:LEN:AVER? ')
        elif( self.testmode == 2 ):
            self.cmd_send(self=self, cmd='CONF:BLU:SIGN1:CONN:PACK:PATT:LEN:LE2M P44 ')
            self.cmd_send(self=self, cmd='INIT:BLU:MEAS1:MEV ')
            recive_data1 = self.CMP200.query('FETC:BLU:MEAS1:MEV:MOD:LEN:LE2M:AVER? ')
            time.sleep(0.1)
            self.stop_mev(self=self)
            self.cmd_send(self=self, cmd='CONF:BLU:SIGN1:CONN:PACK:PATT:LEN:LE2M P11 ')
            self.cmd_send(self=self, cmd='INIT:BLU:MEAS1:MEV ')
            recive_data2 = self.CMP200.query('FETC:BLU:MEAS1:MEV:MOD:LEN:LE2M:AVER? ')
        elif( self.testmode == 4 ):
            self.cmd_send(self=self, cmd='CONF:BLU:SIGN1:CONN:PACK:PATT:LEN:LRAN ALL1 ')
            self.cmd_send(self=self, cmd='INIT:BLU:MEAS1:MEV ')
            recive_data1 = self.CMP200.query('FETC:BLU:MEAS1:MEV:MOD:LEN:LRAN:AVER? ')
            time.sleep(0.1)

            self.cmd_send(self=self, cmd='CONF:BLU:SIGN1:CONN:PACK:PATT:LEN:LRAN ALL1 ')
            self.cmd_send(self=self, cmd='INIT:BLU:MEAS1:MEV ')
            recive_data2 = self.CMP200.query('FETC:BLU:MEAS1:MEV:MOD:LEN:LRAN:AVER? ')
    ...
    def per_search_get_data(self, Level):
        ...
        self.per_data_querry( self=self)
        if ( self.testmode == 1 ):
            recive_data = self.CMP200.query('FETC:BLU:SIGN1:RXQ:PER:LEN? ')
        elif( self.testmode == 2 ):
            recive_data = self.CMP200.query('FETC:BLU:SIGN1:RXQ:PER:LEN:LE2M? ')
        else:
            recive_data = self.CMP200.query('FETC:BLU:SIGN1:RXQ:PER:LEN:LRAN?')
    ...
    def inband_get_data(self,CH):
        ...
        self.cmd_send(self=self, cmd='INIT:BLU:MEAS1:MEV ')
        time.sleep(5)
        recive_data = self.CMP200.query('FETC:BLU:MEAS1:MEV:TRAC:SACP:PTX? ')
    ...
    def hci_aci_get_data(self):
        ...
        self.per_data_querry(self=self)
        if (self.testmode == 1):
            recive_data = self.CMP200.query('FETC:BLU:SIGN:RXQ:PER:LEN? ') #1M OK
        elif (self.testmode == 2):
            recive_data = self.CMP200.query('FETC:BLU:SIGN:RXQ:PER:LEN:LE2M? ') #2M OK
        else:
            recive_data = self.CMP200.query('FETC:BLU:SIGN:RXQ:PER:LEN:LRAN? ')   #S8 OK  S2 OK
    ...
    def aci_data_querry(self):
        ...
        if( self.testmode==1 | self.testmode==2 ):
            ...
            query_timeout = 1000
        while ( is_running!= 'RDY\n' ):
            is_running = self.CMP200.query('FETC:BLU:SIGN:RXQ:PER:STAT? ')
            # print('querry data is ' + is_running)
            time.sleep(0.02)
            nums = nums + 1
            if( nums == query_timeout ):
