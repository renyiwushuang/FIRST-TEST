...
def Prase_2A07_Test(data,len,port):
    ...
    if (bs_id_str in show["figure_bs_snd_uwb_ts"]["show"].keys()) == False:
        show["figure_bs_snd_uwb_ts"]["show"][bs_id_str] = dict()
        show["figure_bs_snd_uwb_ts"]["show"][bs_id_str]["data"] = list()
        show["figure_bs_snd_uwb_ts"]["show"][bs_id_str]["data"].append(bs_uwb_ts)
    else:
        show["figure_bs_snd_uwb_ts"]["show"][bs_id_str]["data"].append(bs_uwb_ts)
...
def Prase_2A08_Test(data,len,port):
    ...
    if (rcv_bs_id in show.keys()) == False:
        show[rcv_bs_id] = dict()
        show[rcv_bs_id]["show"] = dict()
        show[rcv_bs_id]["xmax"] = 1000 
    for i in range(0,num):
        tp = struct.unpack("<HBHBHBIB",data[index:14+index])
        index += 16
        snd_bs_id = "%04x"%tp[0]
        rcv_uwb_ts = tp[5] + tp[6]*256
        if (snd_bs_id in show[rcv_bs_id]["show"].keys()) == False:
            show[rcv_bs_id]["show"][snd_bs_id] = dict()
            show[rcv_bs_id]["show"][snd_bs_id]["data"] = list()
            show[rcv_bs_id]["show"][snd_bs_id]["data"].append(rcv_uwb_ts)
        else:
            show[rcv_bs_id]["show"][snd_bs_id]["data"].append(rcv_uwb_ts)
